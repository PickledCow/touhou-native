...
func attack(t):
	...
	if t % 90 < 60:
		...
		var bullet = Bullets.create_shot_a2(bullet_kit, galacta_position + Vector2(96, 0).rotated(a), 8, a, -0.3334, 0.0, orange_knife, true)
		Bullets.set_bullet_properties(bullet, {"bounce_count": 1, "bounce_surfaces": Constants.WALLS.DOME})
		Bullets.add_transform(bullet, Constants.TRIGGER.TIME, 36, {"accel": 0.5, "max_speed": 8.0})
		a += 0.12 * lr
	...
	# Move
	if (t+1) % 270 == 0:
		set_galacta_dest(Vector2(500 - rand_range(0, 150) * lr, rand_range(200, 300)), 60)
	if (t+1) % 270 == 135:
