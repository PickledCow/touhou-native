...
func _ready():
	...
	item_data.resize(9)
	item_data[0] = 128 * 6			# source x (integer) # (16+8*(c/4))
	...
	item_data[7] = 1					# anim frame, 1 for no animation (integer)
	item_data[8] = 0					# spin
...
func _physics_process(delta):
	if t % 10 == 0 :
		...
		for i in 100:
			#pass
			var bullet = Bullets.create_item(item_kit, item_data, position, rand_range(4, 8), randf()*TAU, (randi()%2-1)*1)
